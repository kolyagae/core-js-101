...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return a < b
    ? `${bracket1}${a}, ${b}${bracket2}`
    : `${bracket1}${b}, ${a}${bracket2}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  if (arr.length % 2 === 0) {
    arr2 = arr.map((el, i) => (i % 2 === 0 ? el * 2 : el));
  } else {
    arr2 = arr.map((el, i) => (i % 2 !== 0 ? el * 2 : el));
  }

  const result = arr2.map((el) => (el > 9 ? el - 9 : el));
...
function getDigitalRoot(num) {
  const num2 = num.toString().split('').map(Number).reduce((acc, curr) => acc + curr, 0);
  if (num < 10) {
    return num;
  }
  return getDigitalRoot(num2);
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    for (let j = 1; j < pathes.length; j += 1) {
      if (pathes[0][i] !== pathes[j][i]) return result.substring(0, result.lastIndexOf('/') + 1);
    }
    result += pathes[0][i];
  }
